...
DWORD CGemMerchantManager::m_dwOpenVIPLevel = CConfigManager::getSingleton()->globalConfig.GemMerchantVIPLevelLimit;
DWORD CGemMerchantManager::m_dwOpenMasterLevel = CConfigManager::getSingleton()->globalConfig.GemMerchantMasterLevelLimit;
float CGemMerchantManager::m_fOpenProbability = CConfigManager::getSingleton()->globalConfig.gemMerchantOpenProbability;

CGemMerchantManager::CGemMerchantManager(CBaseDataManager& baseDataManager)
	: CMallManager(baseDataManager)
	, m_pSpecialMallData(nullptr)
{
	m_fOpenFunc = std::bind(&CGemMerchantManager::OpenProcess, this);
}


CGemMerchantManager::~CGemMerchantManager()
{
	m_pSpecialMallData = nullptr;
	m_pMallData = nullptr;
}

MallType CGemMerchantManager::GetType() const
{
	return MallType::GemMerchant;
}

bool CGemMerchantManager::InitDataMgr(void* pData)
{
	if (nullptr == pData)
		return false;
	...
	m_pVecRefreshClock = &CConfigManager::getSingleton()->globalConfig.vecGemMerchantRefreshClock;
...
GoodsWay CGemMerchantManager::RechargeWay() const
{
	return GoodsWay::gemMan;
}

bool CGemMerchantManager::IsOpened()
{
	/// 如果长期开放 返回true
...
void CGemMerchantManager::OpenProcess()
{
	/// 如果商城已开启则不再触发开启判定
	if (IsOpened())
		return;
...
void CGemMerchantManager::VipLevelChange(int vipLevel)
{
	if (m_pSpecialMallData->bAlwaysOpened)
		return;
	...
	if (vipLevel > m_dwOpenVIPLevel)
	{
		...
		if (g_Script.PrepareFunction("Get_MoudelLimitDetail"))
		{
			...
			g_Script.PushParameter("Gem_OpenLimit");
...
void CGemMerchantManager::LoginProcess()
{
	if (m_pSpecialMallData->bAlwaysOpened)
		return;
...
void CGemMerchantManager::GetCommodityConfigs(vector<CommodityConfig*>& vecConfigs)
{
	CConfigManager::getSingleton()->GetGemMerchantCommodityConfigs(m_pBaseDataMgr.GetPlayerLevel(), vecConfigs);
}

bool CGemMerchantManager::OpenMall(bool alwaysOpen)
{
	if (alwaysOpen)
	{
		lite::Variant ret;
