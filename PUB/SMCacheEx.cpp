    ...
    public SharedMemoryCacheEx
{
    ...
    struct ChainNode
    {
        ...
        DWORD idHolder; // 当前是哪个线程正在持有资源32bit值，高13bit保存线程ID，低19bit保存该线程的唯一性时间戳！
        ...
        void Lock() 
        {
            ...
            if ( tid & 0xffffe000 )
                throw exception( "当前线程由于某种特殊情况，导致线程ID大于8192，无法适应当前的共享内存管理规则！" );

            DWORD idTemp = tid | ( static_cast< DWORD >( reinterpret_cast< unsigned __int64 & >( ct ) / 10000 ) << 13 );
...
_retry:
            ...
            if ( HANDLE threadHandle = OpenThread( THREAD_QUERY_INFORMATION, FALSE, result & 0x1fff ) )
            {
                ...
                if ( ( result & 0xffffe000 ) == 
                    ( static_cast< DWORD >( reinterpret_cast< unsigned __int64 & >( ct ) / 10000 ) << 13 ) )
                {
        ...
        {
            if ( ( node->idHolder & 0x1fff ) != GetCurrentThreadId() )
