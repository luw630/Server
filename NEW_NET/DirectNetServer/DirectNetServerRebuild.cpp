...
struct SClientLink
{
    ...
    DWORD m_dwSendTimes;
    DWORD m_dwRecvTimes;
    DWORD m_dwSendKBytes;
    DWORD m_dwRecvKBytes;

    DWORD m_SendBps;
    DWORD m_RecvBps;
    DWORD m_RecvPrevCheckTime;
};
...
private:
    ...
    tObjectService<DNID, SClientLink> m_LinkManager;
};
...
DNRESULT cDirectNetServer::Host(DNPORT dnPort)
{
    ...
    for (DWORD i = 0; i < m_dwWorkThreadNumber; i++)
    {
        HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, s_iocp_worker_thread_procedure, this, 0, NULL);
    }

    HANDLE hThread = (HANDLE)_beginthreadex(NULL, 0, s_iocp_accept_thread_procedure, this, 0, NULL);
        ...
        case OP_ACCEPT:
            {
                ...
                pOverlapPlus->wbuf.buf = pOverlapPlus->pOperationBuffer.Alloc(sizeof(CRYPTION));
        ...
        case OP_READ_HEAD:
            {
                ...
                else
                {
                    ...
                    pOverlapPlus->wbuf.buf = pOverlapPlus->pOperationBuffer.Alloc(cryption.dnphdr.paclen);
        ...
        case OP_WRITE:
            {
                ...
                {
                    unsigned __int64 *pCheck1 = (unsigned __int64 *)&pOverlapPlus->pOperationBuffer[sizeof(DNPHDR)];
                    unsigned __int64 *pCheck2 = (unsigned __int64 *)"Byebye!";
...
{
    if ( pvBuffer == NULL || dwLength == 0 || 
        dwLength >= ((1 << 16) - sizeof(DNPHDR)) )
    {
        return DNR_E_INVALID_PARAMETER;
    }
    ...
    // 考虑到性能问题，这里不做对象删除，只在recv部分做就可以了，所以不需要dnidClient数据
    OVERLAPPEDPLUS *newolp = new OVERLAPPEDPLUS;
    if (newolp == NULL)
    {
        return DNR_E_OUT_OF_MEMORY;
    }

    memset(newolp, 0, sizeof(OVERLAPPEDPLUS));

    newolp->s = s;
    newolp->dnidClient = dnidClient;
    newolp->OpCode = OP_WRITE;
    newolp->wbuf.len = newolp->dwTotalBytes = sizeof(DNPHDR) + dwLength;
    newolp->wbuf.buf = newolp->pOperationBuffer.m_pEntry = 
        new char[newolp->pOperationBuffer.m_dwSize = (((newolp->wbuf.len >> 3) + 1) << 3)];
    // newolp->wbuf.buf = newolp->pOperationBuffer = new char[((newolp->wbuf.len >> 3) + 1) << 3];
    // newolp->wbuf.buf = newolp->pOperationBuffer.Alloc(((newolp->wbuf.len >> 3) + 1) << 3);

    ((DNPHDR*)(char *)newolp->pOperationBuffer)->seqnum = (WORD)seqnum;
    ((DNPHDR*)(char *)newolp->pOperationBuffer)->paclen = (UINT16)dwLength;

    memcpy(&newolp->pOperationBuffer[sizeof(DNPHDR)], pvBuffer, dwLength);
...
{
    return SendTo(dnidPlayer, "Byebye!", 8);
}
