...
public:
    ...
    BOOL Connect(const char *szIP, WORD wPort, BOOL bBlock = TRUE);
    BOOL Disconnect(); 
...
public:
	...
	//有一服务器加了60秒,所以客服端加60
    DWORD GetSrvTime() { return m_dwSvrTime + ( timeGetTime() - m_dwSvrUpdateTime )/1000 + 60; }
...
private:
    ...
    DWORD volatile m_dwSvrTime;                     // 服务器时钟
    DWORD volatile m_dwSvrUpdateTime;               // 服务器时钟
