...
#define TIMEOUTTIME             (3600000)//1000*60*60 //心跳检测掉线时间
...
BOOL CNetServerEx::Create(const char *szPortName)
{
    ...
    DNRESULT dnr = m_pDirectNetServer->Initialize( ( m_bUseVerify ? &_cryption : NULL ), this, MAX_CLIENT_NUMBER );

    if (DNR_FAILED(dnr))
        return FALSE;

    // this is needed for extracting the port number from a string
    wPort = static_cast<WORD>( ComputHashValue(szPortName) + N_SOCKET_MIN_PORTNUM );
...
BOOL CNetServerEx::Create(const WORD wPort)
{
    ...
    dnr = m_pDirectNetServer->Host(this->wPort = wPort);
...
BOOL CNetServerEx::Close(void)
{
    ...
    __try 
    {
		...
		{
	        SNetServerPacket *pk = m_PacketQueue.front();
            m_PacketQueue.pop();

			if ( pk->m_pvBuffer ) 
				delete pk->m_pvBuffer;
			delete pk;
		}
    }
    __finally 
...
int CNetServerEx::GetLinkByFlags( DNID *dnids, DWORD size, DWORD flags )
{
    __try
    {
        EnterCriticalSection(&m_ClientMapLock);
        ...
        size_t idx = 0;
        for ( std::map<DNID, CLinkData>::iterator it = m_ClientMap.begin();
...
int CNetServerEx::GetQueueSize()
{
    return ( int )m_PacketQueue.size();
}

int CNetServerEx::ReactiveHost()
{
    if ( m_pDirectNetServer )
        return 1;
...
int CNetServerEx::CheckQueueLimit( DWORD maxSize, BOOL netBreak )
{
    DWORD size = 0;
    ...
    if ( size > maxSize )
    {
        if ( netBreak )
        {
            m_pDirectNetServer->Close();
            m_pDirectNetServer->Release();
            m_pDirectNetServer = NULL;
        }
        return 1;
    }
...
void CLinkData::CheckPing( DWORD clientTime )
{
    ...
    else
    {
        ...
        // 如果 服务器的时间 比 客户端的时间快，100秒
        // 或者 客户端的时间 比 服务器的时间快，60秒
        // 则   该客户端可能使用了加速器
        if ( ( margin > TIMEOUTTIME ) || ( margin < ( -PINGMARGIN * 4 ) ) )
        {
            // 如果发现服务器和客户端的时间差超过域值的话，则将连接状态置为无效！
            // 后续的操作则由服务器逻辑进行控制，将该连接状态下的所有消息都直接丢弃！
            m_enumStatus = LinkStatus_UnConnected; 
            OutputDebugString( "timestamp limit break! using time accelerator!\r\n" );
