...
inline BOOL ISUNUSABLE( int type )          { return ( ( type & 1 ) || ( type >= JEWEL_SEGMENT ) ); }
...
inline BOOL ISJEWEL( int type )             { return ( ( type >= JEWEL_SEGMENT ) && ( type < 36 ) ); }
...
{
	DWORD mID;                // ID
	DWORD mLevelAdd[3];       // 等级附加
	DWORD mIntensifyAdd[3];   // 强化附加
	DWORD mHoleAdd[3];        // 洞附加
	DWORD mInnetenessAddOddsAndFlag[2]; // 天生附加几率 和 标识
	DWORD mAptitudeCheckupOdds; // 资质鉴定几率
	DWORD mDropAddsOddsAndFlag[2]; // 掉落附加标识 几率 和标识
};
