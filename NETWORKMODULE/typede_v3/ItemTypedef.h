...
static const int MAX_TASK_NUMBER = 400;
...
//  定时销毁类_可使用，       定时销毁类_不可使用，
    TIME_RECYCLE_USABLE = 10, TIME_RECYCLE_UNABLE = 11,
...
inline BOOL ISUNUSABLE( int type )          { return ( ( type & 1 ) || ( type >= TIME_RECYCLE_UNABLE ) ); }
...
inline BOOL ISTIMERECYCLE( int type )       { return ( type == TIME_RECYCLE_USABLE || type == TIME_RECYCLE_UNABLE ); }

inline BOOL ISJEWEL( int idx )              { return ( ( idx >= 101 ) && ( idx <= 625 ) && ( ( idx % 100 ) <= 25 ) ); }
inline BOOL ISROSE( int idx )               { return ( ( idx >= 51520 ) && ( idx <= 51529 ) ); } // 玫瑰
inline BOOL ISSPECIALPENDANT ( int idx )	{ return ( ( idx == 10725 ) || ( idx == 10727 ) ); } // 同心佩||兰心佩
inline BOOL ISWHINE( int idx )				{ return ( ( idx >= 40300 ) && ( idx <= 40308 ) ); } // 桃酒
...
struct ClientVisible
{
	...
	WORD wSpecial;          // 特殊应用！（目前用于最大的重叠数量,定时回收类 脚本触发编号）
    WORD byLevelEM;         // 等级需求
    WORD wENEM;             // 活力需求
    WORD wINEM;             // 悟性需求
    WORD wSTEM;             // 体魄需求
    ...
    WORD wGraph3;           // 发光图片编号
    BYTE byIsUseCheck;      // 是否使用确认 1确认 0直接使用
    WORD wActiveLevelEM;    // 激活等级需求上限
    WORD wActiveBase;       // 激活基数 75%....
	
    char szName[ITEMNAME_MAXLEN];    //原为11->20

	//该项涉及客户端的显示问题，故将其放在可显示的部分
...
struct ClientInvisible
{
    ...
    WORD stone;                // 
    DWORD dwExpand;            // 扩展，用于不同道具类扩展使用（定时回收类:回收时间/秒）
    
    WORD wWearPrime;          // 现在用来表示耐久度，（以前是：增加效果的持续时间）
};
...
{
	SAutoUseItemSet() :bSetChange(false), bUse(false), wPoint(0), wWhich(0)
	{}
    bool bSetChange;
	bool bUse;
	WORD wPoint;
...
{
    GenItemParams( WORD index, DWORD number = 1 ) : slotNumber( 0 ), level( 1 ), index( index ), number( number ){}
    GenItemParams( WORD index, DWORD number, WORD slotNumber, WORD level = 1 ) : slotNumber( slotNumber ), index( index ), number( number ), level( level ) {}
...
struct SpecialItem
{
    ...
    DWORD mJianDingLevel[ 6 ];     // 鉴定装备的6个类别等级
};
...
#define  _CHECKEX( v )   ( v > 30000 ) ? ( 30000 ) :( v )
        ...
        actionSpeed         = _CHECKEX( data.actionSpeed + actionSpeed );
        moveSpeed           = _CHECKEX( data.moveSpeed + moveSpeed );
